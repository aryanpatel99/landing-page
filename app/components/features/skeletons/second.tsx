...
export const SkeletonSecond = ({
  ...
  return (
    ...
    style={{
     ...
     className={cn('group max-w-[85%] h-full my-auto bg-neutral-100 mx-auto w-full p-3 rounded-xl border border-neutral-300 dark:border-neutral-700 shadow-2xl', "perspective-distant translate-x-8 mask-radial-from-50% mask-b-from-60%","[--pattern-fg:var(--color-neutral-900)]/5 dark:[--pattern-fg:var(--color-white)]/10 flex flex-col ")}>
      <div className="flex gap-3 items-center ">
        <IconCircleDashedCheck className='size-4' />
        <p className='text-sm font-normal'>Campaign Planner</p>
      </div>
      <div className='relative flex-1 mt-4 bg-neutral-200/30 border border-neutral-300 dark:border-neutral-700 rounded-2xl '>
        <Pattern/>
        <div className='absolute translate-x-4 -translate-y-4 group-hover:translate-x-0 group-hover:translate-y-0 transition-all duration-300 inset-0 bg-white h-full w-full divide-y divide-neutral-300 dark:divide-neutral-700 rounded-2xl shadow overflow-hidden'>
...
const Pattern = () => {
  return (
    <div style={{
      "--pattern-fg": "rgba(0,0,0,0.08)",
    } as React.CSSProperties} className='absolute inset-0  bg-[repeating-linear-gradient(315deg,var(--pattern-fg)_0,var(--pattern-fg)_1px,transparent_0,transparent_50%)] bg-size-[10px_10px] bg-fixed'></div>
  )
}